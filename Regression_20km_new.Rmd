...
data2 <- read_csv("regression_20km_new.csv")
#View(data2)

data2$Pool <- as.factor(data2$Pool)
data2$Bar <- as.factor(data2$Bar)
data2$Biz_Center <- as.factor(data2$Biz_Center)
data2$Conference <- as.factor(data2$Conference)
data2$Mtg_Rooms <- as.factor(data2$Mtg_Rooms)
data2$Wifi <- as.factor(data2$Wifi)
data2$Multilingual <- as.factor(data2$Multilingual)
data2$No_Smoke <- as.factor(data2$No_Smoke)
data2$Breakfast_ <- as.factor(data2$Breakfast_)
data2$Air_Trans <- as.factor(data2$Air_Trans)
data2$Free_Parking <- as.factor(data2$Free_Parking)
data2$Beachfront <- as.factor(data2$Beachfront)
data2$Wheelchair <- as.factor(data2$Wheelchair)
data2$Concierge <- as.factor(data2$Concierge)
data2$Laundry <- as.factor(data2$Laundry)
data2$Gym <- as.factor(data2$Gym)
data2$City <- as.factor(data2$City)
data2$dive_site <- as.factor(data2$dive_site)
data2$near_shore <- as.factor(data2$near_shore)
data2$mid_shore <- as.factor(data2$mid_shore)
data2$far_shore <- as.factor(data2$far_shore)

data3 <- data2 %>%
  dplyr::select(2, 4:41, -LATITUDE, -LONGITUDE) %>% 
  mutate(avg_price = (Low_Price + High_Price)/2) %>% 
  clean_names()
names(data3)[5] <- "num_reviews"
